...
eviTrend <- lapply(1:3, function(l)
  apply(evi$eviM[ind[l,],], 2, function(x) {
    quantile(x, probs = c(0.025, 0.5, 0.975), na.rm = T)
  }))
...
eviTrend2 <- lapply(1:3, function(l)
  apply(evi$eviM[ind[l,],which(2008:2020<=2014)], 2, function(x) {
    quantile(x, probs = c(0.025, 0.5, 0.975), na.rm = T)
  }))
...
### Comparison between calcualted green-up date/trends with MODIS MCD12Q2 Land Cover Dynamic values (2008-2018).
...
save(MOD12gup, file = "results/MOD12gup.rda")
...
opar <- par(mar = c(3,3,1,1), mfrow = c(1,3))

rng <- apply(MOD12gup, 2, function(x) quantile(x[x>10], probs = c(0.025, 0.5, 0.975), na.rm = T))

plot(2008:2018, rng[2,], type = "n", ylim = range(rng))
polygon(c(2008:2018, 2018:2008), c(rng[1,], rev(rng[3,])), col = "grey80", border = NA)
lines(2008:2018, rng[2,], lwd = 2, type = "o")

tmp <- data.frame(year = rep(2008:2018, each = nrow(MOD12gup)), 
                        evi = c(MOD12gup),
                        cell = rep(1:nrow(MOD12gup), length(2008:2018)))

      tmp <- subset(tmp, evi>10 & !is.na(evi) & !is.nan(evi) & !is.infinite(evi))
      
      lm <- lmer(evi ~ year + (1|cell), data = tmp[sample(1:nrow(tmp), 25000),], na.action = na.omit)
...
rng <- apply(MOD12gup[coordinates(rOut)[,2]>64,], 2, function(x) quantile(x, probs = c(0.025, 0.5, 0.975), na.rm = T))
plot(2008:2018, rng[2,], type = "n", ylim = range(rng))
polygon(c(2008:2018, 2018:2008), c(rng[1,], rev(rng[3,])), col = "grey80", border = NA)
lines(2008:2018, rng[2,], lwd = 2, type = "o")
